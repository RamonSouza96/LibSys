...
type
  TMovementController = class
    ...
    class function IsStatusMoviment(AId: Integer): Integer;
    class function IsUserAvailable(UserId: Integer): string;
...
begin
  ...
  try
    LSql := 'SELECT E.EMP_ID, ' +
            ...
            '       E.EMP_DATA_RETORNO, ' +
            '       E.EMP_OBSERVACAO, ' +
            '       CASE WHEN E.EMP_DATA_PREVISAO_RETORNO < DATE(''now'') THEN P.PAR_VALOR_MULTA * (JULIANDAY(DATE(''now'')) - JULIANDAY(E.EMP_DATA_PREVISAO_RETORNO)) ELSE 0 END AS EMP_MULTA, ' +
            '       CASE WHEN E.EMP_DATA_PREVISAO_RETORNO < DATE(''now'') THEN 3 ELSE E.EMP_STATUS END AS EMP_STATUS ' +
            'FROM Emprestimos E ' +
...
class function TMovementController.IsStatusMoviment(AId: Integer): Integer;
var
  LCon: TFDConnection;
  LQry: TFDQuery;
begin
  ...
  try
    with LQry do
    begin
      SQL.Text := 'Select EMP_ID, EMP_STATUS from Emprestimos where EMP_ID = :pID';
      ParamByName('pID').AsInteger := AId;
      Open;
      Result := LQry.FieldByName('EMP_STATUS').AsInteger;
    end;
  finally
    LQry.Close;
    LQry.DisposeOf;
...
class function TMovementController.IsUserAvailable(UserId: Integer): string;
var
  LCon: TFDConnection;
  LQry: TFDQuery;
  LSql: string;
begin
  ...
  try
    LSql := ' SELECT USU_ID,'+
            ...
            '        USU_DATA_INATIVO,'+
            '        CASE'+
            '            WHEN USU_ATIVO = 1 THEN "Disponivel"'+
            '            WHEN USU_ATIVO = 0 THEN "Usuário bloqueado ate a data " || strftime(''%d-%m-%Y'', USU_DATA_INATIVO)'+
            '        END AS Status'+
            ' FROM Usuarios'+
            ' WHERE USU_ID = :USU_ID';
    ...
    begin
      SQL.Text := LSql;
      ParamByName('USU_ID').AsInteger := UserId;
      Open;
