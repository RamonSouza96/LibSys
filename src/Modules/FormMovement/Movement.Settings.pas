...
type
  ...
  private
    ...
    const SvgSave =
      ...
      class function ItemExistsInListView(ListView: TListView;
       TagValue: Integer): Boolean;
   public
    const SvgArrowDawn =
     '<?xml version="1.0" encoding="UTF-8"?>'+
     '<svg xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="25" height="25">'+
...
class function TMovementSettings.ItemExistsInListView(ListView: TListView;
  TagValue: Integer): Boolean;
var
  I: Integer;
begin
  ...
  begin
    if ListView.Items[I].Tag = TagValue then
    begin
      Result := True;
      Exit;
    end;
  ...
  case FormMovement.BtnNewMSaveM.tag of
    ...
    1:
      begin
        ...
        if FormMovement.CbxUsuarios.ItemIndex = -1 then
        begin
          ShowMessage('Você precisa adicionar um usuário!');
          Exit;
        end;
...
var
  ...
  CbxLivros: TComboBox;
begin
  LvLivroSaida := FormMovement.LvLivroSaida;
  CbxLivros := FormMovement.CbxLivros;

  if LvLivroSaida.Items.Count = 4 then
  begin
    ShowMessage('Só é permitido adicionar até 4 livros.');
    Exit;
  end;

  LId := CbxLivros.ListItems[CbxLivros.ItemIndex].Tag;
  LNome := CbxLivros.ListItems[CbxLivros.ItemIndex].Text;

  if ItemExistsInListView(LvLivroSaida, LId) then
  begin
    ShowMessage('Este livro já foi adicionado!');
    Exit;
  end;

  with LvLivroSaida.Items.Add do
  begin
    Tag := LId;
...
begin
   if TMovementController.IsStatusMoviment(TSkSvg(Sender).tag) = 1 then
   begin
