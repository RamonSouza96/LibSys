...
begin
  LSelectedItem := FormUser.LvUser.Selected;
  if FormUser.LvUser.Selected <> nil then
  begin
    TUserController.DeleteUser(LSelectedItem.Tag);
  end;
  ClearFields;
  RefreshListView;
...
begin
  if TControl(TEdit(FInputFocus)).Name = 'EdtCadNome' then
  begin
    FormUser.EdtCadConta.SetFocus;
    Exit;
  end;

  if TControl(TEdit(FInputFocus)).Name = 'EdtCadConta' then
  begin
    FormUser.EdtCadSenha.SetFocus;
    Exit;
  end;

  if TControl(TEdit(FInputFocus)).Name = 'EdtCadSenha' then
  begin
...
begin
  ...
  TUser_Settings.RefreshListView;
end;

class procedure TUser_Settings.RefreshListView;
var
  LDataSet: TUserData;
  LItem: TListViewItem;
begin
